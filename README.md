...
For $X_1, \ldots, X_n \sim \mathcal{N}(\theta, \sigma^2)$ with $\sigma$ known, sweeps $\theta \in [-1, 1]$ and approximates the coverage probability of $\left(\bar{X}_n \pm z_{\alpha/2}\dfrac{\sigma}{\sqrt{n}}\right)$ at each point using $M = 1000$ replications, confirming it equals $1 - \alpha$ uniformly.
...
Fixes $\theta_0 = 0$ and varies $n = 1, \ldots, 500$, showing that the coverage probability of

$$
\bar{X}_n \pm z_{\alpha/2}\frac{\sigma}{\sqrt{n}}
$$

remains flat at $1 - \alpha = 0.95$ regardless of sample size, since the pivot
...
**Coverage probability of the asymptotic CI for the Exponential mean.**  
In the following code, it should be noted that the `qnorm()` function has been used. Although the exact sampling distribution of the sample mean is not normal, the quantiles of the normal distribution are used to construct the confidence interval.

The confidence interval has asymptotic confidence coefficient $1-\alpha$. This follows from the fact that, as $n \to \infty$,

$$
\frac{\bar{X}_n - \lambda}{\lambda/\sqrt{n}}
\xrightarrow{d}
\mathcal{N}(0,1),
$$

and therefore the choice of the critical value $z_{\alpha/2}$ is justified for large sample sizes.
```r
alpha = 0.05
n = 10000 # sample size
lambda_0 = 3 # true value 
M = 1000 # number of replications
counter = 0 # increase if contains true value
for (i in 1:M) {
...
**Coverage probability of $[a + X_{(n)},\; b + X_{(n)}]$ depends on $\theta$ — Uniform(0, θ )**
For $X_i \sim \text{Uniform}(0, \theta)$, sweeps $\theta \in [0.5, 3]$ and approximates the coverage of the CI $[a + X_{(n)},\; b + X_{(n)}]$ via $M = 5000$ replications, then overlays the exact probability
...
```r
n = 10 # sample size
theta_vals = seq(0.5, 3, by = 0.05) # discretize parameter space
...
plot(theta_vals, conf_prob, type = "p", pch = 19,
 ...
 cex.main = 1.4, ylab = expression(P[theta](a+X[(n)],b+X[(n)])))
...
**Coverage probability of $[cX_{(n)},\; dX_{(n)}]$ is constant in $\theta$ — Uniform(0, θ)**
For the pivotal CI $[cX_{(n)},\; dX_{(n)}]$ with $1 \leq c < d$, sweeps $\theta \in [0.5, 3]$ and confirms via simulation that the coverage $\left(\dfrac{1}{c}\right)^n - \left(\dfrac{1}{d}\right)^n$ is free of $\theta$, giving a genuine confidence coefficient.
...
Visualises how inverting the acceptance region $|\bar{x} - \mu_0| \leq z_{\alpha/2}\dfrac{\sigma}{\sqrt{n}}$ of a level-$\alpha$ test for $H_0: \mu = \mu_0$ yields the CI $\bar{X}_n \pm z_{\alpha/2}\dfrac{\sigma}{\sqrt{n}}$. Plots the two boundary lines $\bar{x} = \mu \pm z_{\alpha/2}\dfrac{\sigma}{\sqrt{n}}$, marks the acceptance region $A(\mu_0)$, and shows the resulting interval $C(\bar{X})$ for a simulated $\bar{X}$.

```r
alpha = 0.05 # size of the test
cut_off = 2*pnorm(alpha/2) # one sample z test
sigma = 2 # population sd (known)
n = 5 # sample size
mu_0 = 2 # null hypothesis value
plot(0,0, xlim = c(-2, 7), ylim = c(-1,6), xlab = expression(mu),
 ylab = expression(bar(x)), cex = 0.2)
abline(h = 0, lwd = 3, col = "grey")
abline(v = 0, lwd = 3, col = "grey")
curve(x - cut_off*sigma/sqrt(n), add = TRUE, col = "red", 
 lwd = 3, lty = 1)
curve(x + cut_off*sigma/sqrt(n), add = TRUE, col = "red", 
 lwd = 3, lty = 1)
points(mu_0, 0, pch = 19, col = "magenta")
text(mu_0, -0.5, expression(mu[0]))
segments(mu_0, 0, mu_0, mu_0 + cut_off*sigma/sqrt(n), 
 lwd = 3, col = "magenta", lty = 2)
segments(mu_0, mu_0 + cut_off*sigma/sqrt(n), 0, 
 mu_0 + cut_off*sigma/sqrt(n), lwd = 3, 
 col = "magenta", lty = 2)
segments(mu_0, mu_0 - cut_off*sigma/sqrt(n), 0, 
 mu_0 - cut_off*sigma/sqrt(n), 
 lwd = 3, col = "magenta", lty = 2)
...
segments(0, x_bar, x_bar + cut_off*sigma/sqrt(n), x_bar, 
 col = "blue", lwd = 4, lty = 3)
segments(x_bar - cut_off*sigma/sqrt(n), 0, 
 x_bar - cut_off*sigma/sqrt(n), x_bar, lwd = 4, 
 col = "blue", lty = 3)
segments(x_bar + cut_off*sigma/sqrt(n), 0, 
 x_bar + cut_off*sigma/sqrt(n), 
 x_bar, lwd = 4, col = "blue", lty = 3)
...
alpha = 0.05
n = 20 # sample size
lower_CI = numeric(rep) # initialization
upper_CI = numeric(rep) # initialization
for(i in 1:rep){
 x = rnorm(n = n, mean = 0, sd = sqrt(sigma2)) # simulation
...
plot(1:rep, lower_CI, type = "p", pch = 19, col = "grey",
 ylim = c(min(lower_CI), max(upper_CI)), ylab = "95% CI",
 xlab = "replication")
